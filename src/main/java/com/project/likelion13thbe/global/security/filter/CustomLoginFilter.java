...
public class CustomLoginFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(
        ...
        MemberReqDTO.LoginRequest loginRequest;
        try {
            loginRequest = objectMapper.readValue(request.getInputStream(), MemberReqDTO.LoginRequest.class);
        } catch (IOException e) {
            throw new AuthenticationServiceException("[ Login Filter ] Request Body 파싱 과정에서 오류가 발생했습니다.");
        }

        //Request Body 에서 추출
        String email = loginRequest.email(); //Email 추출
        String password = loginRequest.password(); //password 추출
